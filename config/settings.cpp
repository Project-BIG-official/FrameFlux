...
// config/settings.cpp: Exception-Safe INI Synchronizer & Live In-Game Saver
...
void SettingsManager::SaveToFile() {
    try {
        ...
        outFile << "# PB FrameFlux Configuration File\n\n"
                ...
                << "ext_swapchain_maint = " << m_settings.extSwapchainMaint << "\n"
                << "enable_pacing = " << (m_settings.enablePacingWait ? "true" : "false") << "\n";
        outFile.close();

        std::error_code ec;
        m_lastConfigWriteTime = std::filesystem::last_write_time(m_activeConfigPath, ec);
    } catch (...) {}
}
...
void SettingsManager::SyncAndLoadFile(const std::string& path) {
    try {
        ...
        m_settings.extSwapchainMaint = (uint32_t)atoi(getOrDef("ext_swapchain_maint", "0").c_str());
...
void SettingsManager::CheckHotReload() {
    try {
        std::error_code ec;
        if (!std::filesystem::exists(m_activeConfigPath, ec)) return;

        auto currentWriteTime = std::filesystem::last_write_time(m_activeConfigPath, ec);
        if (ec) return;
...
void SettingsManager::LoadOrCreate() {
    try {
        ...
        std::string fallbackDir = std::string(homeEnv ? homeEnv : "/tmp") + "/.config/frameflux";
        std::string fallbackPath = fallbackDir + "/config.ini";

        std::error_code ec;

        // Безопасная проверка: можем ли мы писать в текущую папку игры
        std::ofstream testLocal("frameflux.test", std::ios::out);
        if (testLocal.is_open()) {
            testLocal.close();
            std::filesystem::remove("frameflux.test", ec);
        ...
        // Гарантируем, что ни одно исключение не вылетит в Wine
        ApplyEnvironmentOverrides();
    }
}
