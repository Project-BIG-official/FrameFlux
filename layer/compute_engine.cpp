...
void ComputeEngine::RecordOverlayPass(VkCommandBuffer cmd, VkDescriptorSet descSet, const OverlayPushConstants& pc, uint32_t width, uint32_t height) {
    if (!vk(m_device).CmdBindPipeline) return;
    vk(m_device).CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_overlayPipeline);
    vk(m_device).CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_overlayPipeLayout, 0, 1, &descSet, 0, nullptr);
    vk(m_device).CmdPushConstants(cmd, m_overlayPipeLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(OverlayPushConstants), &pc);
    vk(m_device).CmdDispatch(cmd, (width + 15) / 16, (height + 15) / 16, 1);
}
...
void ComputeEngine::BeginTimestamp(VkCommandBuffer cmd) {
    if (m_queryPool != VK_NULL_HANDLE && vk(m_device).CmdWriteTimestamp) {
        if (vk(m_device).CmdResetQueryPool) {
            vk(m_device).CmdResetQueryPool(cmd, m_queryPool, 0, 2);
        }
        vk(m_device).CmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, m_queryPool, 0);
    }
}

void ComputeEngine::EndTimestamp(VkCommandBuffer cmd) {
    if (m_queryPool != VK_NULL_HANDLE && vk(m_device).CmdWriteTimestamp) {
        vk(m_device).CmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, m_queryPool, 1);
    }
}
...
void ComputeEngine::RecordLumaPass(VkCommandBuffer cmd, VkDescriptorSet descSet, uint32_t width, uint32_t height) {
    if (!vk(m_device).CmdBindPipeline) return;
    vk(m_device).CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_lumaPipeline);
    vk(m_device).CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_lumaPipeLayout, 0, 1, &descSet, 0, nullptr);
    LumaPushConstants pc{width, height};
    vk(m_device).CmdPushConstants(cmd, m_lumaPipeLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(LumaPushConstants), &pc);
    uint32_t packedW = (width + 3) / 4;
    vk(m_device).CmdDispatch(cmd, (packedW + 15) / 16, (height + 15) / 16, 1);
}
...
void ComputeEngine::RecordDownsamplePass(VkCommandBuffer cmd, VkDescriptorSet descSet, uint32_t srcW, uint32_t srcH, uint32_t dstW, uint32_t dstH) {
    if (!vk(m_device).CmdBindPipeline) return;
    vk(m_device).CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_downsamplePipeline);
    vk(m_device).CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_downsamplePipeLayout, 0, 1, &descSet, 0, nullptr);
    DownsamplePushConstants pc{srcW, srcH, dstW, dstH};
    vk(m_device).CmdPushConstants(cmd, m_downsamplePipeLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(DownsamplePushConstants), &pc);
    vk(m_device).CmdDispatch(cmd, (dstW + 15) / 16, (dstH + 15) / 16, 1);
}
...
void ComputeEngine::RecordFlowPass(VkCommandBuffer cmd, VkDescriptorSet descSet, uint32_t width, uint32_t height, uint32_t stride) {
    if (!vk(m_device).CmdBindPipeline) return;
    vk(m_device).CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_flowPipeline);
    vk(m_device).CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_flowPipeLayout, 0, 1, &descSet, 0, nullptr);
    uint32_t gridX = (width + 3) / 4;
    uint32_t gridY = (height + 3) / 4;
    FlowPushConstants pc{gridX, gridY, width, height, stride, 1.0f};
    vk(m_device).CmdPushConstants(cmd, m_flowPipeLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(FlowPushConstants), &pc);
    vk(m_device).CmdDispatch(cmd, gridX, gridY, 1);
}

void ComputeEngine::RecordRefinePass(VkCommandBuffer cmd, VkDescriptorSet descSet, uint32_t width, uint32_t height) {
    if (!vk(m_device).CmdBindPipeline) return;
    vk(m_device).CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_refinePipeline);
    vk(m_device).CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_refinePipeLayout, 0, 1, &descSet, 0, nullptr);
    uint32_t gridX = (width + 3) / 4;
    uint32_t gridY = (height + 3) / 4;
    RefinePushConstants pc{gridX, gridY, width, height};
    vk(m_device).CmdPushConstants(cmd, m_refinePipeLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(RefinePushConstants), &pc);
    vk(m_device).CmdDispatch(cmd, gridX, gridY, 1);
}
...
void ComputeEngine::RecordWarpPass(VkCommandBuffer cmd, VkDescriptorSet descSet, const WarpPushConstants& pc, uint32_t width, uint32_t height) {
    if (!vk(m_device).CmdBindPipeline) return;
    vk(m_device).CmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_warpPipeline);
    vk(m_device).CmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_warpPipeLayout, 0, 1, &descSet, 0, nullptr);
    vk(m_device).CmdPushConstants(cmd, m_warpPipeLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(WarpPushConstants), &pc);
    vk(m_device).CmdDispatch(cmd, (width + 15) / 16, (height + 15) / 16, 1);
}
